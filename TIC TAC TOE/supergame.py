...
def check_winning_move(game_map, player):
...
def get_best_move(game_map, player):
    opponent = 1 if player == 2 else 2
    
    # Block opponent's winning move
    block_move = check_winning_move(game_map, opponent)
    if block_move:
        return block_move
    
    # Play to win
    winning_move = check_winning_move(game_map, player)
    if winning_move:
        return winning_move
...
def play_game_wittictactoe_projecth_computer(game_size):
    ...
    while not game_won:
        ...
        if current_player == 1:  # Human player
            ...
            move = get_best_move(game, current_player)
            if move:
